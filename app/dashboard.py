...
Plotly Dash 4.x application. Dark theme, 4 pages.
...
# ── Layout ────────────────────────────────────────────────────────────────────
app.layout = html.Div([
    dcc.Store(id="page", data="visao-geral"),
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                *[html.Button(
                    ...
                    id=f"nav-{pid}", className="nav-link",
...
@app.callback(
    Output("page", "data"),
    [Input(f"nav-{pid}", "n_clicks") for pid, _, __ in PAGES],
    prevent_initial_call=True,
)
def set_page(*_):
    # ctx.triggered_id is the id of whichever button was clicked
...
@app.callback(
    [Output(f"nav-{pid}", "className") for pid, _, __ in PAGES],
    Input("page", "data"),
)
def highlight_nav(page):
    return [
        "nav-link active" if pid == page else "nav-link"
        for pid, _, __ in PAGES
    ]
...
def render(page):
    if page == "visao-geral":     return _page_overview()
    if page == "inconsistencias": return _page_issues()
    if page == "ativos":          return _page_ativos()
    if page == "assistidos":      return _page_assistidos()
...
def _page_overview():
    ...
    freq = (ALL_ISSUES
            .groupby(["CODIGO", "SEVERIDADE"]).size()
            .reset_index(name="n")
    ...
    fig_bar.update_layout(**LAYOUT, height=260, yaxis_title="Ocorrências")
    ...
    fig_donut.update_layout(**LAYOUT, height=260, showlegend=False)
    ...
    return html.Div([
        ...
        html.Div([
            card("Inconsistências por Código",
                 "Vermelho = CRÍTICO · Laranja = ALERTA",
                 graph(fig_bar)),
...
def _page_issues():
    freq = (ALL_ISSUES
            .groupby(["CODIGO", "SEVERIDADE", "CAMPO"]).size()
            .reset_index(name="n")
            .sort_values("n", ascending=False))
...
def _page_ativos():
    ...
    fig_age.update_layout(**LAYOUT, height=230,
    ...
    fig_sal.update_layout(**LAYOUT, height=230,
                           yaxis_title="Participantes",
                           xaxis_title="Salário (R$)")

    df_sc = df_v[df_v["ID"].notna()]
    fig_sc = go.Figure(go.Scatter(
        x=df_sc["ID"], y=df_sc["SALARIO_CONTRIB"],
        mode="markers",
        marker=dict(color="#388bfd", opacity=0.4, size=5),
    ))
    fig_sc.update_layout(**LAYOUT, height=260,
                          xaxis_title="Idade",
    ...
    fig_cargo.update_layout(
        **LAYOUT, height=260, xaxis_title="Participantes",
        yaxis=dict(autorange="reversed", showgrid=False,
    ...
    return html.Div([
        ...
        html.Div([
            ...
            kpi("Média de Idade",      f"{df['ID'].mean():.1f} anos"),
        ...
        html.Div([
            ...
            card("Distribuição Salarial",
                 "Salário de contribuição (P97 como limite)",
                 graph(fig_sal)),
        ], className="chart-grid"),
...
def _page_assistidos():
    ...
    fig_ben.update_layout(**LAYOUT, height=230,
                           yaxis_title="Assistidos",
                           xaxis_title="Benefício Mensal (R$)")
    ...
    fig_tipo.update_layout(**LAYOUT, height=260, showlegend=False)

    df_sc = df_v[df_v["ID"].notna()]
    fig_sc = go.Figure(go.Scatter(
        x=df_sc["ID"], y=df_sc["BENEFICIO_MENSAL"],
        mode="markers",
        marker=dict(color="#3fb950", opacity=0.5, size=5),
    ))
    fig_sc.update_layout(**LAYOUT, height=260,
                          xaxis_title="Idade",
                          yaxis_title="Benefício Mensal (R$)")
    ...
    return html.Div([
        ...
        html.Div([
            ...
            card("Distribuição de Benefícios",
                 "Benefício mensal (P97 como limite)",
                 graph(fig_ben)),
        ], className="chart-grid"),

        html.Div([
            card("Por Tipo de Benefício",
                 "Aposentadoria programada, pensão, invalidez",
                 graph(fig_tipo)),
